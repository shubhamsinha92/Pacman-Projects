...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
           second_beliefs[self.getJailPosition()] = 1.0
           self.beliefs = second_beliefs
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newP,probability in newPosDist.items():
                second_beliefs[newP] += self.beliefs[oldPos] * probability
        second_beliefs.normalize()
        self.beliefs = second_beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for particle in self.particles:
                ...
                counter[particle] += emissionModel[trueDistance]
                #print "counter[particle]:",particle, counter[particle]
            #counter.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        self.particles = newParticles
            #for newP,probability in newPosDist.items():
                #counter[newP] += particle * probability
        #counter.normalize()
        #self.particles = []
        #self.particles.append(util.sample(counter))
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            ...
            #print "beliefDis",particle, beliefDis[particle]
        beliefDis.normalize()
        return beliefDis
        ...
        for particle in self.particles:
            ...
            for g in range(self.numGhosts):
                if noisyDistances[g] == None:
                    ...
                    trueDistance = util.manhattanDistance(particle[g],pacmanPosition)
                    weight *= emissionModels[g][trueDistance]
            counter1[particle] += weight

        if counter1.totalCount() == 0:
            self.initializeParticles()
        else:
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(util.sample(counter1))
